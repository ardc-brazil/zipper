...
class ZipperService:
    def __init__(self, minio_client: Minio, gatekeeper_gateway: GatekeeperGateway, temp_dir: str):
    ...
    def _zip_files(
        ...
        try:
            ...
            self._gatekeeper_gateway.post_zip_callback(ZippedResource(id=id, dataset_id=dataset_id, version=version, status=ZipStatus.SUCCESS))
    ...
    def zip_files(
        ...
        if not file_names:
            return ZippedResource(id=process_id, dataset_id=dataset_id, version=version, status=ZipStatus.FAILURE)
        ...
        threading.Thread(target=self._zip_files, args=(process_id, dataset_id, version, bucket, file_names, zip_name)).start()

        return ZippedResource(id=process_id, dataset_id=dataset_id, version=version, status=ZipStatus.IN_PROGRESS, bucket=bucket, name=zip_name)
